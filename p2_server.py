...
TIMEOUT = 0.5
DUP_ACK_THRESHOLD = 3
FILE_PATH = "sending_large_file.txt"
...
class TCPRenoServer:
    ...
    def handle_timeout(self):
        logging.info(
            f"Timeout occurred. Old cwnd: {self.cwnd}, ssthresh: {self.ssthresh}"
        )
    ...
    def handle_duplicate_ack(self, seq_num):
        if seq_num not in self.duplicate_acks:
            ...
            self.duplicate_acks[seq_num] += 1
            logging.info(
                f"Duplicate ACK count for {seq_num}: {self.duplicate_acks[seq_num]}"
            )
        ...
        if (
            ...
            logging.info(
                f"Entering fast recovery - cwnd: {self.cwnd}, ssthresh: {self.ssthresh}"
            )
            return True
        return False
